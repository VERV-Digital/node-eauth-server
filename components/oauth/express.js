...
module.exports = function(app, api, User, ens) {
  ...
  app.get('/oauth/user', authenticate(), async function(req, res) {
    ...
    return ens_name ? res.json(Object.assign(req.user.User, {ens: ens_name})) : res.json(Object.assign(req.user.User, {name: req.user.User.address, nickname: req.user.User.address}))
  })
